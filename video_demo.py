...
weights_path = "weights.0100.h5"  # orginal weights converted from caffe
...
vidcap = cv2.VideoCapture('/home/user1/Videos/walk.mp4')
success, test_image = vidcap.read()
count = 0
while success:
    # if count % 10 == 0:
    oriImg = test_image.copy()  # B,G,R order
    ...
    # visualize 1
    canvas = test_image.copy() # B,G,R order
    ...
    cv2.imshow('image', np.uint8(canvas))
    if cv2.waitKey(1) & 0xff == ord('q'):
        break

    # cv2.imwrite('frame_{}.jpg'.format(count), canvas)
    print(count)
    success, test_image = vidcap.read()
    count += 1
